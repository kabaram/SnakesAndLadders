...
public class SnakesAndLadders extends JPanel implements MouseListener{
   private Image board; 
   private GameDie die;
   private static HashMap<Integer, Integer> warps = new HashMap<Integer, Integer>(); //stores the locations and endpoints for snakes and ladders
   ...
   private static ArrayList<Player> players = new ArrayList<Player>(); //collection of players within the game, ranges in size from 2 to 6
   private static HashMap<Integer, Coordinate> locations = new HashMap<Integer, Coordinate>(); //maps a space number to a location on the panel
   ...
   private Timer moveTimer = new Timer (500, new MoveTimer()); //controls the animation of the player advancing along board
   private Timer warpTimer = new Timer(1000, new WarpTimer()); //controls the pause before a player is moved through a warp
   ...
   //Hard codes the locations of the snakes and ladders in the following format (start, end)
   ...
   public void paintComponent(Graphics g){
      ...
      g.setFont(new Font("Arial Black", Font.BOLD, 40));
      g.drawString(players.get(currentPlayer).getName() + "'s Turn!", 1000, 90); //displays the name of the current player
   ...
   //method required for mouse listener interface, handles the clicking of the roll button
   public void mouseClicked(MouseEvent e) {
      int x = e.getX();  //get location of the mouse click
      int y = e.getY();
      if (!rollTimer.isRunning() && !moveTimer.isRunning() && !warpTimer.isRunning()){ //make sure no animation is occurring
      ...
      public void actionPerformed(ActionEvent e){
         if (movesLeft > 0){ //player has moves left
            ...
            if (!moveBackwards){
               ...
               else if (players.get(currentPlayer).getSpaceNum() == 100 && movesLeft == 0){ //player lands on 100 exactly
                  repaint();
                  endGame();
               }
            }
         ...
         //player is done moving
            ...
            if (warps.get(players.get(currentPlayer).getSpaceNum()) != null){ //player landed on a warp
               if (warps.get(players.get(currentPlayer).getSpaceNum()) > players.get(currentPlayer).getSpaceNum()){  //player landed on ladder
                  try { //play fanfare music https://stackoverflow.com/questions/2416935/how-to-play-wav-files-with-java
                     try{
                        clip.stop();
                     }
   ...
   public void endGame(){
      ...
      JOptionPane.showMessageDialog(new JFrame("Congratulations"), players.get(currentPlayer).getName() + " wins!"); //show popup window and exit game
      System.exit(0);
   ...
   private class WarpTimer implements ActionListener{
      public void actionPerformed(ActionEvent e){
         ...
         players.get(currentPlayer).setSpaceNum(warps.get(players.get(currentPlayer).getSpaceNum())); //send player to the mapped warp space
         warpTimer.stop();
         repaint();
